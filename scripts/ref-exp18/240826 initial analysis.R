...
bulk_counts <- read_tsv("/Users/ylin/Downloads/exp18_49f-hi_tec_deconvolution_analysis/cd49f_ctec_analysis/counts/all.tsv")
# add column of sum ctec and mtec gene counts
bulk_counts <- mutate(bulk_counts, ctec_sum = ctec1 + ctec2 + ctec3, mtec_sum = mtec1 + mtec2 + mtec3)
# remove the ".xxx" from the gene names
bulk_counts$gene <- sub("\\..*", "", bulk_counts$gene)
# arrange gene column
...
bulk_counts <- bulk_counts[bulk_counts$gene %in% gene_info[,1], ]

# find duplicate gene entries since there appears to be dimension mismatch
unique_duplicates <- unique(bulk_counts$gene[duplicated(bulk_counts$gene)])
...
est.prop.sum = music_prop(bulk.mtx = bulk_sums.mtx, sc.sce = tec.query.sce, clusters = 'predicted.celltype',
                      samples = 'samples', select.ct = celltype.parse)
est.prop.indiv = music_prop(bulk.mtx = bulk_indiv.mtx, sc.sce = tec.query.sce, clusters = 'predicted.celltype',
                          samples = 'samples', select.ct = celltype.parse)
head(est.prop.sum)
head(est.prop.indiv)

# run MuSiC using parse dataset: ht12 and ht14 only (t.sce) as reference
est.prop.sum = music_prop(bulk.mtx = bulk_sums.mtx, sc.sce = t.sce, clusters = 'predicted.celltype',
                          samples = 'samples', select.ct = celltype.parse)
est.prop.indiv = music_prop(bulk.mtx = bulk_indiv.mtx, sc.sce = t.sce, clusters = 'predicted.celltype',
                            samples = 'samples', select.ct = celltype.parse)
head(est.prop.sum)
head(est.prop.indiv)

# run MuSiC using multiome dataset (tec.ref.sce) as reference
est.prop.sum = music_prop(bulk.mtx = bulk_sums.mtx, sc.sce = tec.ref.sce, clusters = 'celltype',
...
jitter.fig.sum = Jitter_Est(data.matrix(est.prop.sum$Est.prop.weighted),
